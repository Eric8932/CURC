...
from task_template import SYSTEM,META_INFO,CRITERIA,INSTRUCTION_LABEL_GENERATE_PROMPT1,INSTRUCTION_LABEL_GENERATE_PROMPT2,INPUT_PROMPT1,INPUT_PROMPT2,task2task
...
def load_data(data_path,task,max_number,criteria,fewshot_number):
    dataset = []
    ...
    ins_p1 = "现在，请你"+INSTRUCTION_LABEL_GENERATE_PROMPT1[task]
    ins_p2 =  INSTRUCTION_LABEL_GENERATE_PROMPT2[task]
    if fewshot_number>0:
        fewshot_path = os.path.join("/".join(data_path.split("/")[:-1]),"few-shot.json")
        t1_fs,t2_fs,t3_fs,label_l_fs = get_fewshot(fewshot_path,fewshot_number)


    for i in range(len(t1)):
        input_p = INPUT_PROMPT1[task].format(t1[i],t2[i],t3[i])
        full_prompt = [{
            "role":"user",
            "content":""
            }]
         
        full_prompt[0]['content'] = prefix + ins_p1+ins_p2
    
        if fewshot_number<=0:
            full_prompt[0]['content'] += "\n\n"+input_p+"\n"+INPUT_PROMPT2[task]
        else:
            for j in range(fewshot_number):
                if j == 0:
                    full_prompt[-1]['content'] = full_prompt[-1]['content']+"\n\n"+ INPUT_PROMPT1[task].format(t1_fs[j],t2_fs[j],t3_fs[j])+"\n"+INPUT_PROMPT2[task]
                else:
                    full_prompt += [{"role":"user","content":INPUT_PROMPT1[task].format(t1_fs[j],t2_fs[j],t3_fs[j])+"\n"+INPUT_PROMPT2[task]}]
                full_prompt += [{"role":"assistant","content":label_l_fs[j]}]
                full_prompt += [{"role":"user","content":input_p+"\n"+INPUT_PROMPT2[task]}]
...
def main(data_path,task, save_path,max_number,criteria,fewshot_number):

    save_dir = "/".join(save_path.split("/")[:-1])
    Path(save_dir).mkdir(exist_ok=True, parents=True)
            

    data = load_data(data_path,task,max_number,criteria,fewshot_number)
    print(data[0]['prompt'])
